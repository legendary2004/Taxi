...
import { passInput } from "../../interfaces/Input";
import { FaEye, FaEyeSlash } from "react-icons/fa";

function PassInput(props: passInput) {
    return (
        <div>
            ...
            <div className="relative">
                ...
                <div className="absolute inset-y-0 right-0 pe-3.5 flex items-center">
                    {props.isVisible ? <FaEyeSlash className="w-6 h-6 text-gray-500 dark:text-gray-400 hover:cursor-pointer" onClick={() => props.changeVisibility()}/> : <FaEye className="w-6 h-6 text-gray-500 dark:text-gray-400 hover:cursor-pointer" onClick={() => props.changeVisibility()}/>}
                </div>
            </div>
        </div>
    )
